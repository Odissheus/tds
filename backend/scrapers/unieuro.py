    ...
    async def search_product(self, product_model: str, product_brand: str) -> List[PromoResult]:
        ...
        try:
            ...
            product_cards = await page.query_selector_all(
                "div.product-card, article.product, div[data-product-id], .product-tile"
            )

            if not product_cards:
                product_cards = await page.query_selector_all(
                    ".search-result-item, .product-list__item, li.product-item"
                )

            for card in product_cards[:5]:
                try:
                    title_el = await card.query_selector(
                        "h2, h3, .product-title, .product-name, a.product-link"
                    )
                    ...
                    original_price_el = await card.query_selector(
                        ".old-price, .price--old, del, .list-price, .original-price"
                    )
                    ...
                    logger.debug("Error parsing Unieuro card: %s", str(e))
                    continue
            ...
            try:
                ...
                promo_cards = await page.query_selector_all(
                    "div.product-card, article.product, .promo-product"
                )
                ...
                for card in promo_cards[:20]:
                    try:
                        ...
                        promo_price_el = await card.query_selector(
                            ".new-price, .price--new, .sale-price"
                        )
                        ...
                        logger.debug("Error parsing Unieuro promo card: %s", str(e))
                        continue

            except Exception as e:
                logger.warning("Error checking Unieuro promo page: %s", str(e))
